    ...
    public void buildFailurePointer(){
        ...
        while(!queue.isEmpty()){
            ...
            for(Map.Entry<Character,AcNode> characterAcNodeEntry :
                ...
                // pf为null，表示找到了根节点，并且根几点的子节点也没有匹配
                if(pf == null){
                    // 此时直接将节点的失配指针指向根节点
                    child.failure = root;
                }else{  // pf节点的子节点对应的字符串，包含了大年节点的所表示的字符
...
//                                new String(chars,start, temp.depth));
//                    }
//                    temp = temp.failure;
//                }
//            }
...
//    }
    ...
    public List<ParseResult> parseText(String text){
        ...
        for(int i = 0; i<chars.length; i++){
            ...
            // 如果cur不为null，并且当前节点的子节点不包括当前字符，即不匹配
            ...
            if(cur == null){
                ...
                while (temp != null) {
                    ...
                    // 继续判断该节点的失配指针节点，因为失配指针节点表示的模式串是当前匹配的模式串在这些关键词中的最长后缀，且由于当前节点的路径包括了失配指针的全部路径；并且失配指针路径也是一个完整的关键词，需要找出来。
                    temp = temp.failure;
                }
            }
        }
